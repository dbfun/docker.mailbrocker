...
RESP=`swaks --body test-letters/spam-GTUBE.eml --to test@${EXIM_DOMAIN} --from "junk@wrong-domain-name.com" -p${PORT_EXIM} -tls`
...
RESP=`swaks --body test-letters/spam-GTUBE.eml --to test@gmail.com --from "junk@wrong-domain-name.com" --server ${EXIM_DOMAIN} -p${PORT_EXIM} -tls`
assert "$RESP" '=== Connected to'
assert "$RESP" '~> MAIL FROM:'
assert "$RESP" '~> RCPT TO:'
